...
create table if not exists rango_etario (
	id_rango_etario tinyint auto_increment,
    descripcion varchar (25) not null,
    constraint pk_rango_etario primary key (id_rango_etario)
);
...
-- AGREGO LA CONSTRAINT PARA LA COLUMNA QUE CREAMOS --
alter table persona
add constraint fk_rango_etario foreign key (id_rango_etario) references rango_etario (id_rango_etario);

-- TODOS LOS RANGOS A 0 --
update persona
set id_rango_etario = 0;

-- UPDATEAR LOS RANGOS ETARIOS DE LAS PERSONAS CON UN IF --
update persona
set
id_rango_etario = if(edad = 0,1,id_rango_etario),
id_rango_etario = if(edad between 1 and 5,2,id_rango_etario),
id_rango_etario = if(edad between 6 and 12,3,id_rango_etario),
id_rango_etario = if(edad between 13 and 17,4,id_rango_etario),
id_rango_etario = if(edad between 18 and 25,5,id_rango_etario),
id_rango_etario = if(edad between 26 and 40,6,id_rango_etario),
id_rango_etario = if(edad between 41 and 50,7,id_rango_etario),
id_rango_etario = if(edad between 51 and 70,8,id_rango_etario),
id_rango_etario = if(edad between 71 and 90,9,id_rango_etario),
id_rango_etario = if(edad > 91,10,id_rango_etario);
